...
const RestaurantCard = ({ restoCardInfo }) => {
  ...
  return (
    <div className="m-4 p-4 w-72 bg-slate-50 rounded-2xl shadow-lg transform transition-transform duration-300 ease-in-out hover:scale-105 hover:bg-neutral-200" key={id}>
      ...
      <h1 className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-l font-bold">{aggregatedDiscountInfoV3?.header}{aggregatedDiscountInfoV3?.subHeader}</h1>      <h4>{updatedCuisins}</h4>
      <h4>{avgRating}</h4>
      <h4>{costForTwo}</h4>
